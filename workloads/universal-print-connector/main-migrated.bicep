/*
  Universal Print Connector Infrastructure Template (Migrated to Shared Modules)
  
  This template creates a Universal Print Connector infrastructure including:
  - Resource Group with naming pattern RG-{connectorName}
  - Virtual Network with security configurations (using shared module)
  ...
  - Supports both standalone and hub-spoke network configurations
  - Migrated to use shared Bicep modules where possible
*/
...
@description('Deploy in existing VNet (provide VNet resource ID)')
param existingVnetId string

@description('Existing subnet name (if using existing VNet)')
...
var virtualNetworkName = 'vnet-up-${connectorName}-${environment}'
